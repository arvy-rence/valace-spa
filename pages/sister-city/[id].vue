<template>
    <form class="w-full max-w-lg m-auto">
        <div class="py-6 font-khula text-2xl text-center font-bold uppercase text-primary">
            Update City: <span>{{previousCityName}}</span>
        </div>
...
export default {
    ...
    methods: {
        async create(e) {
            ...
            try {
                await axios.put(`/sisterCity/updateSisterCity/${this.$route.params.id}`, city)
                alert("Sister City Successfully Updated")
                this.$router.push('/sister-city')
            } catch (e) {
                alert("Error Updating Sister City")
            }
        },
        clearFields() {
    ...
    async created() {
        ...
        this.previousCityName = data.sisterCity.library_name
    }
}
</script>